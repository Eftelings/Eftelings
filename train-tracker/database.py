...
## Wir suchen nach der Railway Datenbank-URL.
...
if IS_POSTGRES:
    ...
    # Railway nutzt manchmal noch postgres:// statt postgresql://
...
#def get_conn():
    if IS_POSTGRES:
        # Verbindung zur Railway Cloud Datenbank
    ...
    try:
        if IS_POSTGRES:
            with conn.cursor() as cur:
                # PostgreSQL Dialekt (SERIAL statt AUTOINCREMENT, CURRENT_TIMESTAMP)
                cur.execute("""
    ...
    try:
        if IS_POSTGRES:
            with conn.cursor() as cur:
                for j in journeys:
                    try:
                        # PostgreSQL nutzt %s statt ? für Variablen
                        cur.execute("""
